    ...
    def foreign_perdiem_counting(position, country, md):
        ...
        if country_check is None:
            ...
            country_rate = int(input(
                'Какая ставка (введите цифру)? (1. Отельная / 2. Отельная с планом / 3. Обслуживаемые апартаменты  / 4. Необслуживаемые апартаменты): '))
            if country_rate == 1:
                rate_in_country = country_check['Отельная ставка']
            elif country_rate == 2:
                rate_in_country = country_check['Отельная ставка с планом']
            elif country_rate == 3:
                rate_in_country = country_check['Обслуживаемые апартаменты']
            else:
                rate_in_country = country_check['Необслуживаемые апартаменты']
